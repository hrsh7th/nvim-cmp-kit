...
S.format = P.any(
  ...
  P.map(
  P.seq(S.dollar, S.open, S.int, S.colon, S.slash,
  ...
  P.map(
  P.seq(S.dollar, S.open, S.int, S.colon,
    P.seq(S.question, P.opt(P.take_until({ ':' }, { '\\' })), S.colon, P.opt(P.take_until({ '}' }, { '\\' }))), S.close),
    function(values)
      return setmetatable({
        type = Node.Type.FORMAT,
        capture_index = values[3],
  ...
  P.map(P.seq(S.dollar, S.open, S.int, S.colon, P.seq(S.plus, P.opt(P.take_until({ '}' }, { '\\' }))), S.close),
    function(values)
      return setmetatable({
        type = Node.Type.FORMAT,
        capture_index = values[3],
  ...
  P.map(P.seq(S.dollar, S.open, S.int, S.colon, S.minus, P.opt(P.take_until({ '}' }, { '\\' })), S.close),
    function(values)
      return setmetatable({
        type = Node.Type.FORMAT,
        capture_index = values[3],
...
S.transform = P.map(
P.seq(S.slash, P.take_until({ '/' }, { '\\' }), S.slash, P.many(P.any(S.format, S.text({ '$', '/' }, { '\\' }))), S
.slash, P.opt(P.pattern('[ig]+'))), function(values)
  return setmetatable({
...
S.variable = P.any(
  ...
  P.map(P.seq(S.dollar, S.open, S.var, S.colon, P.many(P.any(S.toplevel, S.text({ '$', '}' }, { '\\' }))), S.close),
    function(values)
      return setmetatable({
        type = Node.Type.VARIABLE,
        name = values[3],
...
---@param input string
---@return table
function SnippetText.parse(input)
