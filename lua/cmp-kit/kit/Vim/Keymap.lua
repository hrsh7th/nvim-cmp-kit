...
---@alias cmp-kit.kit.Vim.Keymap.Keys { keys: string, remap?: boolean }
---@alias cmp-kit.kit.Vim.Keymap.KeysSpecifier string|cmp-kit.kit.Vim.Keymap.Keys

---@param keys cmp-kit.kit.Vim.Keymap.KeysSpecifier
---@return cmp-kit.kit.Vim.Keymap.Keys
...
function Keymap.normalize(s)
  local desc = 'cmp-kit.kit.Vim.Keymap.normalize'
...
---Send keys.
---@param keys cmp-kit.kit.Vim.Keymap.KeysSpecifier|cmp-kit.kit.Vim.Keymap.KeysSpecifier[]
...
function Keymap.send(keys, no_insert)
  ...
  return Async.new(function(resolve, _)
    ...
    local callback = Keymap.termcodes(('<Cmd>lua require("cmp-kit.kit.Vim.Keymap")._resolve(%s)<CR>'):format(unique_id))
    if no_insert then
...
function Keymap.to_sendable(callback)
  ...
  return Keymap.termcodes(('<Cmd>lua require("cmp-kit.kit.Vim.Keymap")._resolve(%s)<CR>'):format(unique_id))
end
